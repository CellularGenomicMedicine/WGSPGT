# Haplarithmisis_Phasing_Grandfather - Function to phase when grandfather is referent
...
Haplarithmisis_Phasing_Grandfather <- function(script, Father, Mother, RefSampleID, Gtypes, BAFs, parent, Parent1, EmbryoID, flip){
  ...
  # Source the function to phase using the grandfather
  ...
  # Conditionally replace phased BAFs based on parent parameter
  if(parent == "Father") {
    PhBAF[["M1"]] <- ToRep
    PhBAF[["M2"]] <- ToRep
