PreTestReport_bafplots <- function(script,Chrom,upstream_boundary,downstream_boundary,ChrsLength,ideogram,outPath,colID,fam_members,bafplot,parent){
  ...
  for(sample in colID){
    par(mar=c(0,6,0,1))
    plot(0,ylab="BAF",line=3.5,col="white",main="",xlab="",frame=F,ylim=c(-0.1,1.1),xlim=c(0,ChrLength),cex=Main,cex.lab=Ax,cex.main=Main,cex.axis=Ax,xaxt="n",yaxt="n")
