XYplot <-function(script,logRs,SegLogRs,ChrsLength,ideogram,outPathXY,colID,fam_members){
  ...
  plotCytoXY(BandName=T,ChrsLength,ideogram)
  ...
  for(sample in colID){
    par(mar=c(2,6,0,1))
    plot(0,ylab="LogR",col="white",main="",xlab="",frame=F,ylim=c(-3,3),xlim=c(0,GenomeLength),cex=Ccc,cex.lab=1.2,cex.main=Main,cex.axis=Ax,xaxt="n",yaxt="n")
