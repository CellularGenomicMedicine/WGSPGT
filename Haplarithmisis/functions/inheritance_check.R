inheritance_check <- function(fam_members,RefID,Chrom){
	Father=fam_members[grepl("Father",fam_members[,2]),3]
	Mother=fam_members[grepl("Mother",fam_members[,2]),3]
	Grandmother=fam_members[grepl("Grandmother",fam_members[,2]),3]
	Grandfather=fam_members[grepl("Grandfather",fam_members[,2]),3]
	Sibling=fam_members[grepl("Sibling",fam_members[,2]),3]
	if( RefID=="Grandparents") {
		...
		if (Father=="UF" & Mother=="AM" & Grandfather=="AGF" & Grandmother=="UGM"  ){
			if (Chrom=="X") {
				cat("UF/AM/AGF/UGM with X has not been tested", "\n")
				Unknow=FALSE
			} else {
		...
		if (Father=="UF" & Mother=="AM" & Grandfather=="UGF" & Grandmother=="AGM"  ){
			if (Chrom=="X") {
				cat("UF/AM/UGF/AGM with X has not been tested", "\n")
				Unknow=FALSE
			} else {
