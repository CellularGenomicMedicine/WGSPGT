# Functions for piecewise constant fitting obtained from Nik-Zainal et al. 2012
# Reference:
...
# selectFastPcf - select appropriate Piecewise Constant Fitting
# It calls either runFastPcf or runPcfSubset based on the number of data points per chromosome 
...
# runFastPcf - detecting of breakpoints, compacting signal and fitting using the Potts model to estimate the parameters of a piecewise constant signal
...
# runPcfSubset - divide the data into subsets, perform PCF on each subset, combine results
...
# PottsCompact - Use the Potts filtering algorithm to estimate the values between breakpoints in the compacted data
...
# compact - Calculate number of observations, sums and sums of squares between potential breakpoints.
...
# markWithPotts - Potts filtering on a compact array
...
# findMarks - maps the marks idenfitied in the compressed scale to their corresponding positions in original scale
...
# filterMarkS4 - Identify potential breakpoints in the input data
filterMarkS4 <- function(x, kmin, L, L2, frac1, frac2, frac3, thres){
	...
	## filters (L>L2), then by a filter searching for potential kmin long segments
	lengdeArr <- length(x)
	xc <- cumsum(x)
	xc <- c(0,xc)
	...
	limit <- quantile(cost1B, (1-frac1B), names = FALSE, na.rm = TRUE)
	...
	limit2 <- quantile(cost2, (1 - frac2), names = FALSE, na.rm = TRUE)
	mark2 <- (cost2 > limit2)
	...
	if(kmin > 1){
		...
		limit3 <- quantile(cost1C, (1 - frac1C), names = FALSE, na.rm = TRUE)
