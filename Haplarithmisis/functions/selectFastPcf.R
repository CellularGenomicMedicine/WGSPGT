...
filterMarkS4 <- function(x,kmin,L,L2,frac1,frac2,frac3,thres){
	...
	limit <- quantile(cost1B,(1-frac1B),names=FALSE,na.rm=T)
	...
	limit2 <- quantile(cost2,(1-frac2),names=FALSE,na.rm=T)
	mark2<-(cost2>limit2)
	...
	if(kmin>1){
		...
		limit3 <- quantile(cost1C,(1-frac1C),names=FALSE,na.rm=T)
