Parameters_extraction <- function(outPath,fam_members){
	...
	if(length(Value2_statS)>1) {write.table(Value2_statS,paste(outPath,paste("There are multiple Sibling refs.txt",sep="_"),sep="/"),row.names=FALSE,quote=FALSE,na=''); stop("Multiple Sibling Refs")}
