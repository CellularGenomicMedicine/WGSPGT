# EmbryoTestReportPlot_plotPO - Function to plot the parent of origin

EmbryoTestReportPlot_plotPO <- function(Chrom, dataPo, EmbryoID, Int, ChrLength, Lab, Main, Ax){
  plot(dataPo$Position[dataPo$Chr == Chrom & dataPo[, EmbryoID] > 0], dataPo[dataPo$Chr == Chrom & dataPo[, EmbryoID] > 0, EmbryoID], "h", xaxt = "n", yaxt = "n", xlab = "", xlim = c(0, ChrLength), ylab = "PO", col = "pink", frame = FALSE, ylim = c(-1, 1), cex.lab = Lab, cex.main = Main, cex.axis = Ax)
