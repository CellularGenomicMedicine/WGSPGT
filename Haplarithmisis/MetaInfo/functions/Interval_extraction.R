Intervals_extraction <- function(outPath,samplesheet){
	...
	for(interval in 1:nrow(intervals)){
		for(v1 in Value1_stat) {
			...
			Interval <- data.frame(Chr=gsub("chr","",intervals[interval,1]),Start=intervals[interval,2],Stop=intervals[interval,3],Origin=intervals[interval,4],stringsAsFactors=FALSE)
