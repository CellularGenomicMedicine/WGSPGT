# PreTestReportData_recessive_inf_snps - Function to calulcate informative SNPs for a recessive condition

PreTestReportData_recessive_inf_snps <- function(script, GT, Parent, Father, Mother, REF, Ref, Chroms, Int, fam_members, dbsnp_path, parent, HelixoutPath, Chrom){
  ...
  pretest_list <- list()
	...
	for (Chr in Chroms) {
	  ...
	  # Filter out SNPs where parent information is not available
		...
		# Calculate semi-informative SNPs for recessive condition
		GT_chr_semi_inf <- GT_chr[(GT_chr[, Father] == "AB" & 
		                           GT_chr[, Mother] == "AA" & 
		                           GT_chr[, Ref] != "BB" & 
		                           GT_chr[, Ref] != "NC") | (GT_chr[, Father] == "AB" & 
		                           GT_chr[, Mother] == "BB" & GT_chr[, Ref] != "AA" & 
		                           GT_chr[, Ref] != "NC") | (GT_chr[, Father] == "AA" & 
		                           GT_chr[, Mother] == "AB" & GT_chr[, Ref] != "BB" & 
		                           GT_chr[, Ref] != "NC") | (GT_chr[, Father] == "BB" & 
		                           GT_chr[, Mother] == "AB" & GT_chr[, Ref] != "AA" & 
		                           GT_chr[, Ref] != "NC"), ]
		...
		if (any(grep("S", fam_members[, "Sample_Status"]))){
		...
		if (grepl("AS", Indication_Ref)){
			GT_chr_inf <- snp_origin_call_Affected_sibling(GT_chr_inf, Parent, Parent2, Ref)
		}
		if (grepl("US", Indication_Ref)){
			GT_chr_inf <- snp_origin_call_Unaffected_sibling(GT_chr_inf, Parent, Parent2, Ref)
		}
