# PreTestReportData_Grandparent_inf_snps - Function to calculate informative SNPs when a grandparent is referent
  ...
  for (Chr in Chroms){
    # Subset genetic data for the current chromosome
		...
		GT_chr_semi_inf <- GT_chr[(GT_chr[, Father] == "AB" & GT_chr[, Mother] == "AA") | 
		                          ...
		                          (GT_chr[, Father] == "BB" & GT_chr[, Mother] == "AB") & (GT_chr[, Ref] !="NC"), ]
		...
		if (any(grep("G", fam_members[, "Sample_Status"]))){
			GT_chr_inf <- GT_chr_semi_inf[(GT_chr_semi_inf[, Parent] == "AB" ) & 
			                               (GT_chr_semi_inf[, Ref] != "AB") & 
			                               (GT_chr_semi_inf[, Parent2] != "AB") & 
			                               (GT_chr_semi_inf[, Ref] != "NC"), ]
		}
		...
		# Call appropriate function based on indication of reference
		# When referent is affected:
		if (grepl("AG", Indication_Ref)) { 
		  GT_chr_inf <- snp_origin_call_Affected_grandparent(GT_chr_inf, Parent, Parent2, Ref) 
		}
		# When referent is unaffected:
		if (grepl("UG", Indication_Ref)) { 
		  GT_chr_inf <- snp_origin_call_Unaffected_grandparent(GT_chr_inf, Parent, Parent2, Ref) 
		} 
